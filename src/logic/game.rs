...
fn gravity(state: &mut State) {
    ...
    for row in (0..FIELD_HEIGHT).rev() {
        for col in 0..FIELD_WIDTH {
            ...
            if tile == EMPTY || is_connected(state, row, col, &id_manager::get_unique_id(tile)) {
                continue;
            }
...
mod tests {
    ...
    fn test_gravity() {
        ...
        let l_composite_id = id_manager::create_composite_id(
            &8,
            &id_manager::next_unique_id(&mut state.used_ids, &8),
        );

        let p_composite_id = id_manager::create_composite_id(
            &9,
            &id_manager::next_unique_id(&mut state.used_ids, &9),
        );

        let x_composite_id = id_manager::create_composite_id(
            &0,
            &id_manager::next_unique_id(&mut state.used_ids, &0),
        );
